...
class Register {
  ...
  cashGreaterPrice(customCash){
    ...
    //收銀機的錢剛好等於找的錢
    if(totalDrawerCash === change){
      let sameValueIndex = this.drawerCash.findIndex((cash) => cash[1] === change)
      //如果有剛好相同幣值
      if(sameValueIndex !== -1){
        this.cidEqualChange(sameValueIndex,change)
      }
      //不同幣值
      else{
  ...
  cidEqualChange(sameValueIndex,change){
    changeDue.style.display = 'block'
    changeDue.innerHTML = `Status: CLOSED ${this.drawerCash[sameValueIndex][0]}: $${this.drawerCash[sameValueIndex][1]}`
    this.drawerCash[sameValueIndex][1] = (Math.round(this.drawerCash[sameValueIndex][1] * 100) - Math.round(change * 100)) / 100
    this.updateCid()
  }
  ...
  cidGreaterChange(change){
    ...
    for(let cash of drawerCashReverse){
      ...
      console.log(originCashIndex)
      if(drawerCashReverse.indexOf(cash) === drawerCashReverse.length -1 && cashTotal < change){
        this.cidLessChange()
        this.updateCid()
        return
      }
